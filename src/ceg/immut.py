...
def add_none(v: tuple[Optional[T], ...]) -> tuple[Optional[T], ...]:
    return v + (None,)

def set_tuple_add(d: frozendict[K, tuple[V, ...]], k: K, v: V) -> frozendict[K, tuple[V, ...]]:
...
def set_tuple(t: tuple[T, ...], i: int, v: T, default: T) -> tuple[T, ...]:
    # if already equal return
    if i > len(t):
        t = t + tuple((
            default for _ in range(i - len(t) - 1))) + (v,)
