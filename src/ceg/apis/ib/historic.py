...
class daily_bar(daily_bar_kw, core.Node.Col1D):
    ...
    # TODO: want some kind of init on at least the first call, eg. to load an iter with the relevant starting date? so we don't hvae to do at=d calls each time

    def __call__(
        self, event: core.Event, graph: core.Graph
    ):
        ds = graph.select(self.d, event)
        d = ds[-1]
        # TODO: ref col needs to point to date? store as int and decode?

        # TODO: get bar given date
...
class daily_bars_kw(NamedTuple):
    ...
    # TODO: flag for skip none or not, ie. size window in observations or in days
